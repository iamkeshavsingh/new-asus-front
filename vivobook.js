...
document.addEventListener("DOMContentLoaded", function () {
  const slides = [
    {
      image: "assets/vivobook/banner.png",
      title: "Vivobook Pro",
      subtitle: "Ultimate Designer",
      buttonText: "Buy Now",
    },
    {
      image: "assets/vivobook/banner.png",
...
// dealsSwiper/colorSwiper/bestSellingSwiper/chooseOffersSwiper/
// productSwiper). Handles the two problems that padding otherwise causes:
...
// [data-clone] excluded: those are the mobile-only loop-filler duplicates of
// card 1/card 6 (see index.html) — they must not count towards the real
// card total, the indicator dots, or budgetCards[0]'s measured width (they're
// display:none on desktop, so measuring one there would read 0).
...
function updateBudgetCarousel() {
  ...
  if (isMobilePeekView && budgetProductsContainer) {
    ...
    // The mobile track is [clone-of-last][card 1]...[card N][clone-of-first]
    // (spacers are display:none here), so a real card's own position is
    // always one step in from the start — no spacerWidth term needed, unlike
    // the desktop branch below which still has real spacers in its flow.
    const trackPosition = budgetCurrentIndex + 1;
...
function loopBudgetCarousel(direction) {
  ...
  // Track order is [clone-of-last(0)][card 1(1)]...[card N(N)][clone-of-first(N+1)].
  // The leading clone (position 0) can't be perfectly centered — there's
  // nothing before it to shift right into — so this clamps like
  // updateBudgetCarousel() does; a template string can't safely hold a
  // negative value here anyway (`-${-54.6}` renders as the invalid
  // "--54.6px", which silently no-ops and leaves transitionend never
  // firing below).
  const cloneTrackPosition =
    direction === "next" ? budgetTotalCards + 1 : 0;
  const cloneMoveAmount = Math.max(
    0,
    cloneTrackPosition * (cardWidth + gap) - (containerWidth - cardWidth) / 2,
  );

  budgetCurrentIndex = direction === "next" ? 0 : budgetTotalCards - 1;
  budgetCarousel.style.transform = `translateX(-${cloneMoveAmount}px)`;
  updateBudgetIndicators();
...
// excluded — those are the mobile loop-filler duplicates (see index.html)
...
// size, and loop seamlessly using the [data-clone] slides in index.html
...
// as the Shop by Budget carousel's loopBudgetCarousel(), adapted for native
// scroll-snap instead of a JS transform.
(function () {
  ...
  // so they move exactly in step with the finger/scroll — see the `transition:
  // none` on these properties in vivobook.css, which is what makes inline
  // style updates apply with zero lag instead of easing behind each frame.
  const MIN_SCALE = 0.82;
  const MAX_SCALE = 1;
  // Below this, opacity is already ~0 (imperceptible), so flipping
  // display:none here doesn't read as a pop — see vivobook.css for why this
  // replaced a max-height collapse (that clipped .bottom-content, which
  // needs to render wider than the card, and made the reveal look like a
  // top-down "wipe" as the height grew).
  ...
  function updateCenterSlide() {
    ...
    slides.forEach((slide) => {
      ...
      if (content) {
        ...
        content.style.display = progress > VISIBLE_THRESHOLD ? "" : "none";
        content.style.pointerEvents = progress > 0.6 ? "auto" : "none";
      }
